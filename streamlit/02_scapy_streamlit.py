...
# 패킷 5개 캡쳐 후 출력
# 캡쳐한 패킷이 streamlit 콘솔에 출력
# sniff(count=5, prn=lambda x:print(x))
st.markdown('### 패킷 5개 캡쳐 후 출력')
...
# 패킷 캡쳐 후 데이터 프레임으로 출력
st.markdown('### 패킷 캡쳐 후 데이터 프레임으로 출력')
packets = sniff(filter="tcp", count=5)
# for p in packets:
    # st.text(p.time)  - 타임 스탬프 형식으로 보여줌
    # st.text(p[0].src)
    # st.text(p[0].dst)
    # st.text(p[0].summary())
data = []
