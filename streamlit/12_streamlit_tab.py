...
with tab2:
    ...
    # 본문의 대화 상자
    # 다른 말로는 모달 대화상자라고도 함
    # 모달 대화상자 : 대화상자가 열려있는 동안 사용자는 다른 작업을 할 수 없음
    # 예) 메모장에서 '파일-열기' 대화상자를 띄운 경우
    # 모달리스 대화상자 : 대화상자가 열려있는 동안 상용자는 다른 화면 조작 가능
    # 예) 메모장에서 '편집-찾기' 대화상자를 띄운 경우
    # 스트림릿의 세션 관리
    # 스트림릿은 스크립트를 매번 위에서 다시 실행하는 구조임 - 기존 값은 없어짐
    ...
    def dialog_box():
        ...
        if st.button('Submit'):
            ...
            st.rerun() # 이벤트 발생 시 ui를 새로 그리게 함
...
with tab5:
    ...
    if go_download:
        ...
        def download_file(url, filename):
            # 파이썬 용 http 요청 라이브러리
            # 웹 사이트에 있는 정보를 요청(get) 혹은, 정보를 전송(post)
            # 쿠키, 헤더, 인증 등 웹과 관련된 복잡한 작업을 수행함
            # 사용 시 브라우저 헤더를 반드시 설정할 것
            ...
            with open(filename, "wb") as f:
                ...
                # 보통 파일이나 데이터를 한번에 통채로 처리하지 않고 나눠 처리할 때 사용
                # 특히 큰 파일이나 스트리밍 시 유용하게 사용
                # chunk_size=8192 -> 다운로드 할 파일을 8kb chunk 단위로 나눠 응답을 받음
        ...
        download_file('https://www.fitpetmall.com/wp-content/uploads/2023/10/shutterstock_1844153299-1024x683-1.png',
                      'cat.jpg')