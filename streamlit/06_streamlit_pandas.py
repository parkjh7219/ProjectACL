...
# 데이터프레임 필터링 (변형)
# 블리언 인덱싱 (행 필터링)
# 데이터 프레임[데이터 프레임['컬럼명'] 조건식]
# 첫 번째 컬럼의 값이 0보다 큰 것만 골라서 출력
df = df[df['Column 2'] > 0]
...
# 첫 번째/세 번째 컬럼의 값이 0.5보다 큰 것만 골라서 출력
...
# 첫 번째 컬럼의 값이 0~2 사이인 값만 선택
df = df.query('`Column 1` >= 0 and `Column 1` <= 2')
# df = df.between(df['Column 1'].between(1.0, 2.0))
st.subheader('Mutated DataFrame(col1 : 첫 번째 컬럼의 값이 0~2 사이인 값만 출력)')
st.dataframe(df)

# 'Column 1'을 기반으로 새 컬럼 'Column 4' 생성
# 람다 함수 혹은 람다 식이라고 함 
# 람다 함수 : 이름 없이(익명) 한 줄로 쓰는 간단한 함수
...
# 변형된 데이터프레임 표시
st.subheader('Mutated DataFrame(col4 : 람다 함수로 생성)')
st.dataframe(df)
